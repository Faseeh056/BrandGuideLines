<script lang="ts">
	...
	async function analyzePrompt(prompt: string) {
		...
		try {
			...
			// Build feedback message showing what was extracted
			let feedback = '**Analysis Complete!**\n\n';
			const found: string[] = [];

			if (collectedInfo.brandName) found.push(`Brand name: **${collectedInfo.brandName}**`);
			if (collectedInfo.industry) found.push(`Industry: **${collectedInfo.industry}**`);
			if (collectedInfo.style) found.push(`Style: **${collectedInfo.style}**`);
			if (collectedInfo.audience) found.push(`Audience: **${collectedInfo.audience}**`);

			if (found.length > 0) {
				feedback += '**Extracted from your prompt:**\n' + found.map(f => `- ${f}`).join('\n') + '\n\n';
			}
			...
			const missingCount = essentialQuestions.length;

			if (missingCount > 0) {
				const missingNames = essentialQuestions.map(q => {
					if (q.id === 'brandName') return 'brand name';
					if (q.id === 'industry') return 'industry';
					if (q.id === 'style') return 'visual style';
					return q.id;
				});
				feedback += `**Still need:** ${missingNames.join(', ')}\n\n`;
				feedback += "I'll ask a few quick questions to fill in the gaps.";
			} else {
				feedback += "I have all the essential info! Let me ask a few follow-up questions.";
			}

			await addBotMessage(feedback);
			await delay(400);
